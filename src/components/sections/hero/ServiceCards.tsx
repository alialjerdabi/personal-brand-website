 ...
 * parity. On touch/mobile the cards stack and the proof is always
 * visible; nothing is hover-gated.
 */
export default function ServiceCards({ services }: ServiceCardsProps) {
  return (
    <ul className="flex flex-col gap-4 sm:h-[26rem] sm:flex-row lg:h-[30rem]">
        ...
        >
          ...
          >
            <Image
              ...
              className="object-cover motion-safe:transition-transform motion-safe:duration-500 sm:group-hover:scale-105 sm:group-focus-within:scale-105"
            />
            <span
              aria-hidden="true"
              className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/25 to-black/10 motion-safe:transition-opacity motion-safe:duration-500"
            />
